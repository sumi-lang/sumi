...
- `tests/corpus/` at the workspace root is the file-based corpus: each case is a directory holding `case.sumi` and, beside it, `frontend.snap` recording the tree, the parser's evidence, the diagnostics, the fixed source, and the normalized source. The runner is `crates/frontend/tests/corpus.rs`. To add a case, write its `case.sumi` and run `UPDATE_FRONTEND=1 cargo test -p sumi-frontend --test corpus`; review every snapshot change as part of the diff.
