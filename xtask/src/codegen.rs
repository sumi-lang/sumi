...
/// punctuation token and of every compound operator, so byte-level
/// mutation can splice in whole tokens instead of discovering each one a
/// byte at a time. Rust's string escaping is the dictionary's.
pub fn dictionary(grammar: &Grammar) -> String {
    ...
    lines.extend(
        grammar
            ...
            .map(|token| format!("{}={:?}", token.name, text(token))),
    );
    lines.extend(
        grammar
            .compounds
            .iter()
            .map(|compound| format!("{compound:?}")),
    );
