...
use crate::{BinaryOp, ExprId, ExprKind, FunctionId, LocalId};
...
pub(crate) fn check(
    ...
    for c in 0..count {
        ...
        match found {
            Some((choice, lax)) => {
                let mut lo: Option<crate::Int> = None;
                let mut hi: Option<crate::Int> = None;
                let mut finite = true;
...
fn delta(
    ...
    match &body.exprs[expr.index()].kind {
        ...
        ExprKind::Binary {
            ...
            .map(|(p, c)| (p, c.add(may(*rhs))))
            .or_else(|| {
                delta(body, typing, lets, *rhs, depth + 1).map(|(p, c)| (p, may(*lhs).add(&c)))
            }),
        ExprKind::Binary {
            op: BinaryOp::Sub,
            lhs,
            rhs,
        } => delta(body, typing, lets, *lhs, depth + 1).map(|(p, c)| (p, c.sub(may(*rhs)))),
        ExprKind::If {
            then_branch,
